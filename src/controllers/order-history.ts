  ...
  try {
    ...
    const orders = pastOrders.map(order => ({
      ...
      pickupAddress: order.deliveryAddress === 'Pickup from store'
        ? `${order.orderProducts[0].product.seller.line1}, ${order.orderProducts[0].product.seller.line2}, ${order.orderProducts[0].product.seller.city}, ${order.orderProducts[0].product.seller.district}`
        : null,
    }));
